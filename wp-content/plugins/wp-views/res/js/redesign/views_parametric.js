...
WPViews.ParametricSearchFilters = function( $ ) {
	...
	self.add_control_structure_on_insert = function( shortcode_data ) {
			...
			if ( 'insert' == shortcode_gui_action ) {
				...
				if ( 
					...
					if ( 'bootstrap' == output_type ) {
						shortcode_string_for_between_min = '<div class="form-group">' +  "\n\t" + '<label>' + shortcode_label + '</label>' + shortcode_string_for_between_min + "\n" + '</div>';
						shortcode_string_for_between_max = '<div class="form-group">' +  "\n\t" + '<label>' + shortcode_label + '</label>' + shortcode_string_for_between_max + "\n" + '</div>';
					} else {
						shortcode_string_for_between_min = shortcode_label + shortcode_string_for_between_min;
						shortcode_string_for_between_max = shortcode_label + shortcode_string_for_between_max;
					}
					...
					var shortcode_label = _.has( shortcode_raw_atts, 'shortcode_label' ) ? ( '[wpml-string context="wpv-views"]' + shortcode_raw_atts.shortcode_label + '[/wpml-string]' ) : '';
					if ( 'bootstrap' == output_type ) {
						shortcode_string = '<div class="form-group">' + "\n\t" + '<label>' + shortcode_label + '</label>' + shortcode_string + "\n" + '</div>';
					} else {
		...
		if ( 'wpv-control-post-relationship' == shortcode_name ) {
			...
			_.each( filter_ancestors, function( ancestor, index ) {
				...
				if ( 'bootstrap' == output_type ) {
					shortcode_ancestor = '<div class="form-group">' + "\n\t" + '<label>' + shortcode_label + '</label>' + shortcode_ancestor + "\n" + '</div>';
				} else {
					shortcode_ancestor =  "\n\t" + shortcode_label + shortcode_ancestor;
				}
	...
	self.adjust_control_post_relationship_dialog = function( data ) {
		var tree_select		= $( '#wpv-control-post-relationship-ancestors' ),
			tree_empty		= $( '<option>', { value: '', text: wpv_parametric_i18n.generic.select_one } ),
			ancestors		= [],
